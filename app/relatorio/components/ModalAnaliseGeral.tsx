...
type BalancoResponse = {
    ...
    need_login?: number;
    msg?: string;
    atendimentos?: BalancoAtendimento[];
};
...
function periodoParaBalanco(aDe?: string, aAte?: string): { inicio: string; fim: string } {
    if (aDe && aAte) {
        return aDe <= aAte
            ? { inicio: aDe, fim: aAte }
            : { inicio: aAte, fim: aDe };
    ...
    if (aDe) return { inicio: aDe, fim: aDe };
    if (aAte) return { inicio: aAte, fim: aAte };

    // Sem filtro explícito, mantém a ideia de "todo o histórico" do analítico.
    return { inicio: "1970-01-01", fim: hojeIso() };
}

async function listarAtendimentosBalanco(aDe?: string, aAte?: string): Promise<BalancoAtendimento[]> {
    const { inicio, fim } = periodoParaBalanco(aDe, aAte);
    const url = `${BALANCO_URL}?inicio=${encodeURIComponent(inicio)}&fim=${encodeURIComponent(fim)}`;
    ...
    const json = (await response.json().catch(() => null)) as BalancoResponse | null;

    if (!response.ok || !json || json.ok === false) {
        throw new Error(json?.msg || `Falha HTTP ${response.status} ao consultar o balanço.`);
    }

    return Array.isArray(json.atendimentos) ? json.atendimentos : [];
}

function getBalancoId(r: BalancoAtendimento): string {
    return String(r.atendimento_id ?? r.id ?? "").trim();
}
...
export default function ModalAnaliseGeral({
    ...
    const [atendimentosBalanco, setAtendimentosBalanco] = React.useState<BalancoAtendimento[]>([]);
    ...
    const carregar = React.useCallback(async () => {
        ...
        try {
            ...
            /*
             * O balanço é a fonte autoritativa da quantidade de atendimentos do período.
             * Ele usa a mesma regra do backend:
             * COALESCE(MIN(historico.datahora), MIN(est_movimento.criado_em)).
             *
             * O analítico continua sendo carregado para ornamentação, arrumação e
             * demais campos operacionais que não fazem parte do resumo do balanço.
             */
            const [lista, listaBalanco] = await Promise.all([
                listarAnalitico(),
                listarAtendimentosBalanco(aDe, aAte),
            ]);

            if (requisicao !== requisicaoAnaliticoRef.current) return;

            setDados(Array.isArray(lista) ? lista : []);
            setAtendimentosBalanco(Array.isArray(listaBalanco) ? listaBalanco : []);
        } catch (e: any) {
            if (requisicao !== requisicaoAnaliticoRef.current) return;

            setErro(e?.message || "Falha ao carregar dados da análise.");
            setDados([]);
            setAtendimentosBalanco([]);
        } finally {
     ...
     * o balanco.php já devolve somente os atendimentos cuja data_referencia
     * pertence ao intervalo solicitado. Portanto não repetimos no React a regra
     * antiga que escolhia data/data_inicio_velorio/ultima_datahora.
     *
     * Mesclamos os dados do balanço com o registro analítico correspondente.
     * Assim a quantidade e os convênios permanecem corretos mesmo quando
     * listarAnalitico() não contém um atendimento já finalizado/recolhido.
     */
    const dadosPorId = React.useMemo(() => {
    ...
    const dadosPeriodo = React.useMemo(() => {
        return (atendimentosBalanco || []).map((atendimento) => {
            const id = getBalancoId(atendimento);
            const base = id ? dadosPorId.get(id) : undefined;

            return {
                ...(base || {}),
                id: (base as any)?.id ?? atendimento.id ?? atendimento.atendimento_id,
                atendimento_id:
                    (base as any)?.atendimento_id ??
                    atendimento.atendimento_id ??
                    atendimento.id,
                falecido:
                    atendimento.falecido ||
                    (base as any)?.falecido ||
                    "",
                convenio:
                    atendimento.convenio ??
                    (base as any)?.convenio ??
                    "",
                data_referencia: atendimento.data_referencia,
                assistencia_ativa: atendimento.assistencia_ativa,
                tanatopraxia_ativa: atendimento.tanatopraxia_ativa,
            } as RegistroAnalise;
        });
    }, [atendimentosBalanco, dadosPorId]);

    const registrosComEventoNoPeriodo = atendimentosBalanco.length;
    ...
    const tanatoEntities = React.useMemo(() => {
        ...
        // Inclui também atendimentos do balanço marcados com tanatopraxia,
        // inclusive os que já não aparecem na listagem operacional.
        for (const r of atendimentosBalanco || []) {
            if (!r.tanatopraxia_ativa) continue;

            const id = getBalancoId(r);
            if (!id) continue;

            if (!entityMap.has(id)) entityMap.set(id, new Set());
            entityMap.get(id)!.add(id);
        }
    ...
    const {
        ...
        const assist = (atendimentosBalanco || []).reduce(
            (total, r) => total + (r.assistencia_ativa ? 1 : 0),
            0
        );
        ...
        for (const atendimento of atendimentosBalanco || []) {
            const convTxt = String(atendimento.convenio || "").toLowerCase();
    ...
    }, [dadosPeriodo, atendimentosBalanco, rangeTanato, cacheVersion, tanatoEntities, falecidoByEntity]);
