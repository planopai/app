...
function maskWhatsApp(phone?: string) {
    ...
    // Formatos comuns no BR: +55 (DD) 9XXXX-XXXX (13 dígitos sem +) ou fixo (12)
    if (only.length < 12) return phone; // fallback
    const country = only.slice(0, 2);
    const ddd = only.slice(2, 4);
    // Se houver 13+ dígitos apos país+ddd, usa 5-4; senão 4-4
...
function classNames(...xs: Array<string | undefined | false | null>) {
    return xs.filter(Boolean).join(' ');
}

// Escapa HTML e aplica formatação inline simples com segurança
...
export default function ClubePublicPage() {
    ...
    const categoriasComContagem = useMemo(
        () =>
            CATEGORIAS.map((c) => ({
                ...c,
                total: PARCEIROS.filter((p) => p.ativo && p.categorias.includes(c.id)).length,
            })),
        []
    );
        ...
        return PARCEIROS.filter((p) => p.ativo)
            ...
            .filter((p) => {
                ...
                const txt = [
                    ...
                    p.descricaoCurta,
                    ...(p.tags ?? []),
                    ...p.categorias.map((id) => CATEGORIAS.find((c) => c.id === id)?.nome ?? ''),
                ]
                    .join(' ')
                    .toLowerCase();
    ...
    // foco no search ao abrir (UX opcional)
    useEffect(() => {
    ...
    return (
        <div className="container">
            ...
            <header className="topbar">
                <div className="brand" aria-label="Marca e título do clube">
            ...
            <section className="categories" aria-label="Filtros por categoria">
                <button
            ...
            {/* Grid de parceiros */}
            <section className="grid" aria-label="Lista de parceiros">
        ...
        * { box-sizing: border-box; }
        .container { background: var(--bg); color: var(--text); min-height: 100vh; }
        .topbar {
          position: sticky; top: 0; z-index: 10; display: grid; grid-template-columns: 260px 1fr;
          align-items: center; gap: 16px; padding: 16px 24px; background: linear-gradient(0deg, #0ea5e9, #0ea5e9); color: #fff;
        }
        .brand { display: flex; align-items: center; gap: 12px; font-size: 18px; }
        .brand strong { font-weight: 800; letter-spacing: 0.5px; }
        .brand span { opacity: 0.9; font-weight: 500; }
        .logo { width: 36px; height: 36px; border-radius: 12px; display: grid; place-items: center; background: rgba(255, 255, 255, 0.2); }
        .search input { width: 100%; padding: 12px 16px; background: #fff; color: #111827; border-radius: 999px; border: 0; outline: none; }

        .categories { display: flex; gap: 10px; padding: 18px 24px; overflow-x: auto; background: #f8fafc; border-bottom: 1px solid var(--border); }
        .chip { display: inline-flex; align-items: center; gap: 8px; padding: 8px 12px; border-radius: 999px; background: var(--chip); border: 1px solid rgba(99, 102, 241, 0.15); font-size: 14px; cursor: pointer; transition: all 0.15s ease; white-space: nowrap; }
        .chip:hover { transform: translateY(-1px); background: var(--chip-active); }
        .chip--active { background: var(--chip-active); border-color: #60a5fa; box-shadow: inset 0 0 0 1px #93c5fd; }
        .chip--disabled { opacity: 0.45; cursor: not-allowed; }
        .chip-icon { font-size: 16px; }
        .chip-count { padding: 2px 8px; border-radius: 999px; background: #e5e7eb; color: #111827; font-size: 12px; }

        .grid { padding: 24px; display: grid; grid-template-columns: repeat(12, 1fr); gap: 18px; }
        ...
        .card { grid-column: span 3; background: var(--card); border: 1px solid var(--border); border-radius: var(--radius); box-shadow: var(--shadow); overflow: hidden; display: grid; grid-template-rows: auto 1fr; cursor: pointer; transition: transform 0.16s ease, box-shadow 0.16s ease; }
        .card:hover { transform: translateY(-2px); box-shadow: 0 10px 24px rgba(2, 6, 23, 0.14); }
        .card-media { padding: 18px; display: grid; place-items: center; background: #f8fafc; border-bottom: 1px solid var(--border); }
        .card-media img { max-width: 140px; height: 72px; object-fit: contain; filter: saturate(0.96); }
        .card-body { padding: 14px 16px 18px; }
        .card-title { margin: 0 0 6px 0; font-size: 16px; }
        .card-desc { margin: 0; color: var(--muted); font-size: 14px; }
        .badge { display: inline-block; margin-top: 10px; padding: 6px 10px; font-weight: 700; border-radius: 8px; background: #ecfeff; color: #0369a1; border: 1px solid #bae6fd; }
        .tags { display: flex; gap: 8px; flex-wrap: wrap; margin: 12px 0 0; padding: 0; list-style: none; }
        .tags li { font-size: 12px; color: #334155; background: #f1f5f9; border: 1px solid #e2e8f0; padding: 4px 8px; border-radius: 999px; }
 ...
 *  Modal de detalhes
 *  ======================= */
function DetalheParceiro({ parceiro, onClose }: { parceiro: Parceiro; onClose: () => void; }) {
    ...
    return (
        <div className="overlay" onClick={onClose} role="dialog" aria-modal="true" aria-label={`Detalhes do parceiro ${parceiro.nome}`}>
            <div className="sheet" onClick={(e) => e.stopPropagation()}>
                <header className="sheet-header">
                    <div className="sheet-brand">
                        <img src={parceiro.logoUrl || 'https://via.placeholder.com/160'} alt={parceiro.nome} />
                        <div>
                            <h2>{parceiro.nome}</h2>
                ...
                <div className="sheet-content">
                    <section>
                        ...
                        <p className="highlight">
                            {b.descontoPercentual ? `${b.descontoPercentual}% OFF` : b.descontoValor ? `R$ ${b.descontoValor} OFF` : b.titulo}
                        </p>
                            ...
                            <ul className="enderecos">
                                {parceiro.enderecos.map((e, i) => (
                                    <li key={i}>
                                        ...
                                        <span>
                                            {e.rua}
                                            {e.numero ? `, ${e.numero}` : ''} {e.complemento ?? ''} – {e.bairro ?? ''} • {e.cidade}/{e.uf} {e.cep ? `• CEP ${e.cep}` : ''}
                                        </span>
                                        {e.mapaUrl && (
                                            <a href={e.mapaUrl} target="_blank" rel="noopener noreferrer">Ver mapa</a>
                                        )}
                                    </li>
                                ))}
                            </ul>
                    ...
                    <section>
                        ...
                        <ul className="contatos">
                            ...
                            {parceiro.contato.email && (
                                <li>
                                    ✉️ <a href={`mailto:${parceiro.contato.email}`}>{parceiro.contato.email}</a>
                                </li>
                            )}
                            {parceiro.contato.instagram && (
                                <li>
                                    📸{' '}
                                    <a href={parceiro.contato.instagram} target="_blank" rel="noopener noreferrer">Instagram</a>
                                </li>
                            )}
                        </ul>
                    </section>
            ...
            <style jsx>{`
        .overlay { position: fixed; inset: 0; background: rgba(2, 6, 23, 0.55); display: grid; place-items: center; padding: 20px; }
        .sheet { width: 100%; max-width: 920px; background: #fff; border-radius: 16px; box-shadow: 0 24px 60px rgba(2, 6, 23, 0.4); overflow: hidden; animation: pop 0.15s ease; }
        @keyframes pop { from { transform: translateY(8px); opacity: 0.7; } to { transform: translateY(0); opacity: 1; } }
        .sheet-header { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 16px 18px; border-bottom: 1px solid #e5e7eb; background: #f8fafc; }
        .sheet-brand { display: flex; align-items: center; gap: 14px; }
        .sheet-brand img { width: 56px; height: 56px; object-fit: contain; border-radius: 12px; background: #fff; border: 1px solid #e5e7eb; padding: 6px; }
        .sheet-brand h2 { margin: 0; font-size: 18px; }
        .sheet-brand p { margin: 4px 0 0; color: #64748b; font-size: 14px; }
        .close { border: 0; background: #0ea5e9; color: #fff; border-radius: 10px; width: 36px; height: 36px; cursor: pointer; }
        .sheet-content { padding: 16px 18px 22px; display: grid; grid-template-columns: 1fr 1fr; gap: 18px 24px; }
        @media (max-width: 840px) { .sheet-content { grid-template-columns: 1fr; } }
        .sheet-content section h3 { margin: 0 0 6px; font-size: 15px; }
        .highlight { display: inline-block; font-weight: 800; padding: 6px 10px; border-radius: 8px; background: #ecfeff; color: #0369a1; border: 1px solid #bae6fd; }
        .bullets { margin: 0; padding-left: 18px; }
        .bullets li + li { margin-top: 6px; }
        .enderecos { display: grid; gap: 10px; margin: 0; padding: 0; list-style: none; }
        .enderecos li { border: 1px solid #e5e7eb; border-radius: 10px; padding: 10px 12px; background: #fbfdff; }
        .enderecos li strong { display: block; margin-bottom: 4px; }
        .enderecos li a { display: inline-block; margin-top: 6px; color: #0ea5e9; }
        .contatos { list-style: none; padding: 0; margin: 0; display: grid; gap: 8px; }
        .muted { color: #64748b; }
