...
export default function UsuariosPage() {
    ...
    const cargoLabel = (slug?: string | null) => {
        const value = String(slug ?? '').trim();
        if (!value) return 'Sem cargo';

        return cargos.find((c) => c.slug === value)?.nome || value;
    };

    const save = async () => {
        setError(null);

        if (!nome.trim() || !usuario.trim() || !cargo.trim() || (editingId == null && !senha.trim())) {
            setError(
                editingId == null
                    ? 'Preencha nome, usuário, senha e cargo.'
                    : 'Preencha nome, usuário e cargo.'
            );
            return;
        }
            ...
            <div className="flex items-center justify-between gap-3 mb-6 md:mb-8">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        Cadastro de usuários e vínculo com cargos.
                    </p>
                </div>
                ...
                <table className="w-full table-fixed min-w-[720px]">
                    <thead>
                        <tr className="bg-gray-50 text-left">
                            ...
                            <th className="px-4 py-4 w-32">Ação</th>
                        </tr>
                    </thead>
                ...
                footer={
                    <>
                        ...
                        <button
                            ...
                            disabled={loading}
                        >
                            {loading ? 'Salvando...' : editingId == null ? 'Adicionar' : 'Salvar'}
                        </button>
                    </>
                }
            >
                    ...
                    <div>
                        ...
                        {cargos.length === 0 && (
                            <p className="mt-1 text-xs text-amber-700">
                                Nenhum cargo ativo foi retornado pelo backend.
                            </p>
                        )}
                    </div>
