...
export default function AcompanhamentoPage() {
    ...
    const fetchRegistros = useCallback(() => {
        fetch("/informativo.php?listar=1&_nocache=" + Date.now(), {
            cache: "no-store",
            headers: {
                Pragma: "no-cache",
                Expires: "0",
    ...
    const fetchAvisos = useCallback(() => {
        fetch("/avisos.php?listar=1&_nocache=" + Date.now(), { credentials: "include" })
            .then((r) => r.json())
    ...
    const enviarAviso = useCallback(async () => {
        ...
        const res = await fetch("/avisos.php", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
