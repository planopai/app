...
export default function PermissoesPage() {
    ...
    const fetchPaginas = async () => {
        try {
            const res = await fetch(`${API_URL}?action=list_pages`, { cache: 'no-store' });
            if (res.ok) {
                const data = (await res.json()) as Pagina[];
                if (Array.isArray(data) && data.length) setPaginas(data);
            }
        } catch {
            /* usa default local */
        }
    ...
    const fetchPerms = async (uid: number) => {
        ...
        setMsg(null);
        try {
            const res = await fetch(`${API_URL}?action=list_permissions&user_id=${uid}`, { cache: 'no-store' });
            const data = await res.json();
            if (!res.ok) throw new Error(data?.erro || 'Falha ao obter permissões.');
            const set: Record<string, boolean> = {};
            (data as string[]).forEach((k) => (set[k] = true));
            setAllowed(set);
        } catch (e: any) {
    ...
    useEffect(() => {
        if (userId != null) fetchPerms(userId);
    }, [userId]);
    ...
    const save = async () => {
        ...
        setError(null);
        try {
            const selecionadas = Object.entries(allowed)
                .filter(([, v]) => v)
                .map(([k]) => k);

            const res = await fetch(`${API_URL}?action=save_permissions`, {
                method: 'POST',
            ...
            if (!res.ok || data?.erro || data?.sucesso === false) {
                throw new Error(data?.msg || data?.erro || 'Falha ao salvar permissões.');
            }
